...
void send_packet_pthread(char * pt_send){
	send_packet(send_payload, ((struct sender_info *)pt_send)->num, ((struct sender_info *)pt_send)->bpf_s);
}

void recv_packet_pthread(char * pt_recv){
	recv_packet(send_payload, ((struct sender_info *)pt_recv)->num, ((struct sender_info *)pt_recv)->bpf_s);
}
...
for(i=0;i<10;i++){
	...
	struct sender_info pt_send;
	struct sender_info pt_recv;
	pt_send.num = 2000;
	pt_send.bpf_s = bpf_send_s;
	pt_recv.num = 1;
	pt_recv.bpf_s = bpf_recv_s;

	gettimeofday(&tv_start,NULL);

	pthread_create(&pt_send,NULL,(void *)send_packet_pthread,(char*) &pt_send);
	pthread_create(&pt_recv,NULL,(void *)recv_packet_pthread,(char*) &pt_recv);
	
/*
	send_packet(send_payload,2000, bpf_send_s);
	recv_packet(recv_payload,1, bpf_recv_s);
*/
	pthread_joint(pt_recv,NULL);

	gettimeofday(&tv_end,NULL);

	pthread_joint(pt_send,NULL);
